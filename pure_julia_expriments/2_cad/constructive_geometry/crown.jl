# based on: https://github.com/plut/ConstructiveGeometry.jl/blob/master/examples/crown.jlusing ConstructiveGeometry
using WGLMakie
using Colors
...
function hyperboloid(r, (x,y,z))
	θ = y/r
	k = 1+(z+17.4)^2/2e4
	return [(x+r)*cos(θ)*k, (x+r)*sin(θ)*k, z]
end

N = 8
R = N*lis_w/π-.01
def = deform(p->hyperboloid(R, p))
distance2=(p,q)->ConstructiveGeometry.wrapsagitta(R, p,q)
lis3d_def = def*lis3d
